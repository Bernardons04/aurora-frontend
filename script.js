    ...
    methods: {
        ...
        canBuy(poder, tipoLinha) {
            ...
            if (index > -1) {
                ...
                if (habIndex > -1) {
                    this.personagem.habilidades.splice(habIndex, 1);
                }

                this.save(); // Salva automaticamente ao alterar poderes
                return;
            }
            if (this.canBuy(poder, tipoLinha)) {
                this.personagem.poderes.push(poder.nome);
                
                // Adiciona à lista de Habilidades se ainda não estiver lá
                if (!this.personagem.habilidades.includes(poder.nome)) {
                    this.personagem.habilidades.push(poder.nome);
                }

                this.save(); // Salva automaticamente
